...
- Add comments where appropriate to explain certain parts of code. Add a doctring to classes and functions to explain their functionality. Finally check if the code follows a proper literature structure so that it is easier to read and understand. Keep a consistent naming convention. Make sure variables, functions, classes, etc. all have a specific naming style of each of them.
- Containers can only run certain tasks. They can be stopped when they're not being used and started again whenever those tasks need to run.
